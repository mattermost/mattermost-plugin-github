...
type Client struct {
	...
	getOrganization func(organizations string) ([]string, map[string]bool)
}

// NewClient creates and returns Client. The third party package that queries GraphQL is initialized here.
func NewClient(logger pluginapi.LogService, getOrganization func(organizations string) ([]string, map[string]bool), token oauth2.Token, username, orgName, enterpriseBaseURL string) *Client {
