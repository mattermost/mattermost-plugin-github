...
func (p *Plugin) handleSubscribesAdd(_ *plugin.Context, args *model.CommandArgs, parameters []string, userInfo *GitHubUserInfo) string {
	...
	if len(githubHooks) == 0 {
		...
		msg += fmt.Sprintf("\n`/github webhook add %s [content-type:application-json/application/x-www-form-urlencoded] [features:create/delete..]  [ssecure-ssl:true/false]`", owner)
		msg += fmt.Sprintf("\n`/github webhook add %s/%s [content-type:application-json/application/x-www-form-urlencoded] [features:create/delete..]  [ssecure-ssl:true/false]`", owner, repo)
	}
	return msg
}
