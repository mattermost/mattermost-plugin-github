...
{{else}}{{end}}`))
	...
	template.Must(masterTemplate.New("helpText").Parse("" +
		...
		"  * `/github webhook add owner[/repo] [events]` - Add a new webhook to your organization or repository to post webhook events to the Mattermost GitHub plugin.\n" +
		"  	 * `events` is a comma-delimated can have one or more values `options:value1,value2`, default *\n" +
